MCAROH2 ; GENERATED FROM 'MCARHOLT2' PRINT TEMPLATE (#980) ; 09/19/10 ; (FILE 691.6, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 G A1R
A1 ;
 S X=$G(^MCAR(691.6,D0,1,D1,0)) W ?17 S Y=$P(X,U,1) S Y=$S(Y="":Y,$D(^MCAR(695,Y,0))#2:$P(^(0),U),1:Y) S Y=$S(Y="":Y,$D(^PSDRUG(Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,30)
