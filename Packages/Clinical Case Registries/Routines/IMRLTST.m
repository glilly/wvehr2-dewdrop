IMRLTST ;HCIOFO/FAI-LOOKUP LAB TEST VALUES ;11/07/01  09:39;
 ;;2.1;IMMUNOLOGY CASE REGISTRY;**9,5,16**;Feb 09, 1998
...
PLBS Q:LRES=""
 ...
 S ^TMP($J,TYP,LDT,LRES,DNAM)=LNM_U_GRP_U_LDR_U_DTRC_U_DTAA
 Q
...
SORT I '$D(^TMP($J)) W !,"**NO DATA FOUND**" Q
 S TY=""
 S I="" F  S I=$O(^TMP($J,I)) Q:I=""  S T="" F J=0:0 S T=$O(^TMP($J,I,T)) Q:T'>0  S G="" F K=0:0 S G=$O(^TMP($J,I,T,G)) Q:G=""  S H="" F M=0:0 S H=$O(^TMP($J,I,T,G,H)) Q:H=""  D SCR
 Q
SCR S RC=^TMP($J,I,T,G,H),LN=$P(RC,U,1),IMDATE=$P(RC,U,5)
 W:TY'=I !
 W !,$E(I,1,14),?17,IMDATE,?41,$E(LN,1,23),?61,$E($J(G,15),1,20)
 S TY=I
 Q
KILL ; kill variables
 K ^TMP($J),CDAR,CDP,HVR,IMDATE,IMRCD4,IMRCD4D,IMRCD4E,IMRCD4X,IMRCDX,IMRCDXD,IMREDIT,IMRESULT,IMRLOOP,IMRPN,X,Y
