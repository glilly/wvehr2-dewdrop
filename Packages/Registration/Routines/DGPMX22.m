...
DE S DIE="^DGPM(",DIC=DIE,DP=405,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGPM(DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,5) S:%]"" DE(5)=% S %=$P(%Z,U,6) S:%]"" DE(6)=%,DE(14)=% S %=$P(%Z,U,7) S:%]"" DE(15)=% S %=$P(%Z,U,13) S:%]"" DE(1)=%,DE(9)=%
 I $D(^("USR")) S %Z=^("USR") S %=$P(%Z,U,3) S:%]"" DE(18)=% S %=$P(%Z,U,4) S:%]"" DE(20)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X3 S Y="@25"
 Q
4 S DQ=5 ;@21
5 S DW="0;5",DV="RP4'X",DU="",DLB="ASIH FACILITY",DIFLD=.05
 S DE(DW)="C5^DGPMX22"
 S DU="DIC(4,"
 G RE
...
C5S S X="" G:DG(DQ)=X C5F1 K DB
 ...
 I $P(^DGPM(DA,0),"^",2)=3 S A1B2TAG="ADM" D ^A1B2XFR
C5F1 Q
X5 I '$D(DGPMT) W !?3,*7,"USE BED CONTROL MOVEMENT OPTIONS!" K X
 Q
 ;
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW="0;6",DV="R*P42'X",DU="",DLB="WARD LOCATION",DIFLD=.06
 S DE(DW)="C6^DGPMX22"
 S DU="DIC(42,"
 S X=$P(DGPM0,"^",6)
 S Y=X
...
9 D:$D(DG)>9 F^DIE17,DE S DQ=9,DW="0;13",DV="DX",DU="",DLB="ABSENCE RETURN DATE",DIFLD=.13
 ...
 G RD
X9 K:'$D(DGPMT) X I $D(X) S %DT="X" D ^%DT S X=Y K:Y<1!(Y<$P($P(^DGPM(DA,0),"^",1),".",1)) X I $D(X) D ABSRET^DGPMVDD
 Q
 ;
...
14 S DW="0;6",DV="R*P42'X",DU="",DLB="WARD LOCATION",DIFLD=.06
 S DE(DW)="C14^DGPMX22"
 S DU="DIC(42,"
 G RE
...
C14S S X="" G:DG(DQ)=X C14F1 K DB
 ...
 S Y=^DGPM(DA,0) I +Y,Y<DT S Y=$P(Y,U,2) I Y<3,$D(DGOWD) S DGHNYT=$S(Y=1:10,1:12) D ^DGPMGLC K DGIDX
C14F1 Q
X14 S DIC("S")="I $S($D(^(""ORDER"")):^(""ORDER""),1:0)" D W^DGPMVDD I $D(X) D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X I $D(X) D WARD^DGPMVDD K:$D(DGOOS) X K DGOOS
 Q
 ;
15 D:$D(DG)>9 F^DIE17,DE S DQ=15,DW="0;7",DV="*P405.4'X",DU="",DLB="ROOM-BED",DIFLD=.07
 S DE(DW)="C15^DGPMX22"
 S DU="DG(405.4,"
 G RE
...
C15F1 Q
X15 K:'$D(DGPMT) X I $D(X) S DIC("S")="I $D(^DG(405.4,""W"",+$P(^DGPM(DA,0),""^"",6),+Y)) D OCC^DGPMRB I 'DGPMOC" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X I $D(X) D ROOM^DGPMVDD K:$D(DGOOS) X K DGOOS
 Q
 ;
16 S DQ=17 ;@25
17 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=17 D X17 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X17 I DGPMP=^DGPM(DA,0) S Y=""
 Q
18 D:$D(DG)>9 F^DIE17,DE S DQ=18,DW="USR;3",DV="RP200'",DU="",DLB="LAST EDITED BY",DIFLD=102
 S DU="VA(200,"
 S X=DUZ
 S Y=X
 S X=Y,DB(DQ)=1,DE(DW,"4/")="" G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
