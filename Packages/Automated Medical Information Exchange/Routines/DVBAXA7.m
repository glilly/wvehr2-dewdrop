...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 I $D(^(.11)) S %Z=^(.11) S %=$P(%Z,U,1) S:%]"" DE(6)=% S %=$P(%Z,U,2) S:%]"" DE(7)=% S %=$P(%Z,U,3) S:%]"" DE(8)=%
 I $D(^(.32)) S %Z=^(.32) S %=$P(%Z,U,3) S:%]"" DE(5)=%
 I $D(^(.36)) S %Z=^(.36) S %=$P(%Z,U,1) S:%]"" DE(3)=%
 I $D(^(.52)) S %Z=^(.52) S %=$P(%Z,U,5) S:%]"" DE(4)=%
 I $D(^("TYPE")) S %Z=^("TYPE") S %=$P(%Z,U,1) S:%]"" DE(1)=%
 I $D(^("VET")) S %Z=^("VET") S %=$P(%Z,U,1) S:%]"" DE(2)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW="TYPE;1",DV="P391'a",DU="",DLB="TYPE",DIFLD=391
 S DE(DW)="C1^DVBAXA7",DE(DW,"INDEX")=1
 S DU="DG(391,"
 G RE
...
C1F1 N X,X1,X2 S DIXR=664 D C1X1(U) K X2 M X2=X D C1X1("O") K X1 M X1=X
 I $G(X(1))]"" D
 . K ^DPT("APTYPE",X,DA)
...
C1F2 Q
X1 Q
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW="VET;1",DV="SXa",DU="",DLB="VETERAN (Y/N)?",DIFLD=1901
 S DE(DW)="C2^DVBAXA7"
 S DU="Y:YES;N:NO;"
 G RE
...
C2F1 Q
X2 I $D(X) S:'$D(DPTX) DFN=DA D:'$D(^XUSEC("DG ELIGIBILITY",DUZ)) VAGE^DGLOCK:X="Y" I $D(X) D:$D(DFN) EV^DGLOCK
 Q
 ;
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW=".36;1",DV="*P8'XaR",DU="",DLB="PRIMARY ELIGIBILITY CODE",DIFLD=.361
...
C3F1 N X,X1,X2 S DIXR=743 D C3X1(U) K X2 M X2=X D C3X1("O") K X1 M X1=X
 D
...
X3 S DFN=DA D EV^DGLOCK I $D(X) D ECD^DGLOCK1
 Q
 ;
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW=".52;5",DV="RSXR",DU="",DLB="POW STATUS INDICATED?",DIFLD=.525
...
C4S S X="" G:DG(DQ)=X C4F1 K DB
 ...
 S X=DG(DQ),DIC=DIE
 D EVENT^IVMPLOG(DA)
C4F1 N X,X1,X2 S DIXR=646 D C4X1(U) K X2 M X2=X D C4X1("O") K X1 M X1=X
 D
...
C4X1(DION) K X
 ...
 Q
C4F2 Q
X4 S DFN=DA D POWV^DGLOCK I $D(X) D SV^DGLOCK
 Q
 ;
5 D:$D(DG)>9 F^DIE17,DE S DQ=5,DW=".32;3",DV="*P21'XaR",DU="",DLB="PERIOD OF SERVICE",DIFLD=.323
 S DE(DW)="C5^DVBAXA7"
 S DU="DIC(21,"
 G RE
...
X5 S DFN=DA D POS^DGLOCK1
 Q
 ;
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW=".11;1",DV="FaR",DU="",DLB="STREET ADDRESS [LINE 1]",DIFLD=.111
...
C6S S X="" G:DG(DQ)=X C6F1 K DB
 D ^DVBAXA9
C6F1 N X,X1,X2 S DIXR=230 D C6X1(U) K X2 M X2=X D C6X1("O") K X1 M X1=X
 D
...
C6X1(DION) K X
 ...
 Q
C6F2 Q
X6 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>35!($L(X)<3) X
 I $D(X),X'?.ANP K X
 Q
 ;
7 D:$D(DG)>9 F^DIE17,DE S DQ=7,DW=".11;2",DV="Fa",DU="",DLB="STREET ADDRESS [LINE 2]",DIFLD=.112
...
C7S S X="" G:DG(DQ)=X C7F1 K DB
 D ^DVBAXA11
C7F1 N X,X1,X2 S DIXR=232 D C7X1(U) K X2 M X2=X D C7X1("O") K X1 M X1=X
 D
...
C7X1(DION) K X
 ...
 Q
C7F2 Q
X7 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>30!($L(X)<3) X D:$D(X) UP^DGHELP
 I $D(X),X'?.ANP K X
 Q
 ;
8 D:$D(DG)>9 F^DIE17,DE S DQ=8,DW=".11;3",DV="Fa",DU="",DLB="STREET ADDRESS [LINE 3]",DIFLD=.113
 S DE(DW)="C8^DVBAXA7",DE(DW,"INDEX")=1
 G RE
C8 G C8S:$D(DE(8))[0 K DB
 D ^DVBAXA12
C8S S X="" G:DG(DQ)=X C8F1 K DB
 D ^DVBAXA13
C8F1 N X,X1,X2 S DIXR=233 D C8X1(U) K X2 M X2=X D C8X1("O") K X1 M X1=X
 D
...
C8X1(DION) K X
 ...
 Q
C8F2 Q
X8 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>30!($L(X)<3) X
 I $D(X),X'?.ANP K X
 Q
 ;
9 D:$D(DG)>9 F^DIE17 G ^DVBAXA14
