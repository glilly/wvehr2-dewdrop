MCAROK ; GENERATED FROM 'MCAREKG1' PRINT TEMPLATE (#976) ; 09/19/10 ; (FILE 691.5, MARGIN=80)
 G BEGIN
N W !
...
E1R ;
 ...
 G F1R
F1 ;
 S X=$G(^MCAR(691.5,D0,2,D1,0)) D N:$X>17 Q:'DN  W ?17 S Y=$P(X,U,1) S Y=$S(Y="":Y,$D(^MCAR(695,Y,0))#2:$P(^(0),U),1:Y) S Y=$S(Y="":Y,$D(^PSDRUG(Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,30)
