...
DE S DIE="^DGCR(399,",DIC=DIE,DP=399,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGCR(399,DA,""))=""
 I $D(^("U1")) S %Z=^("U1") S %=$P(%Z,U,3) S:%]"" DE(1)=% S %=$P(%Z,U,10) S:%]"" DE(4)=%
 I $D(^("U2")) S %Z=^("U2") S %=$P(%Z,U,4) S:%]"" DE(9)=% S %=$P(%Z,U,5) S:%]"" DE(12)=% S %=$P(%Z,U,6) S:%]"" DE(15)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW="U1;3",DV="FX",DU="",DLB="OFFSET DESCRIPTION",DIFLD=203
 G RE
X1 K:$L(X)>24!($L(X)<3) X
 I $D(X),X'?.ANP K X
 Q
...
X3 I $P(^DGCR(399,DA,"U1"),"^",11)']"" S Y="@66"
 Q
4 S DW="U1;10",DV="RNJ10,2",DU="",DLB="*FY 1 CHARGES",DIFLD=210
 G RE
X4 S:X["$" X=$P(X,"$",2) K:X'?.N.1".".2N!(X>9999999)!(X<0) X
 Q
 ;
5 S DQ=6 ;@66
6 S DQ=7 ;@67
...
X8 S:'$D(^DGCR(399,DA,"I1")) Y="@672"
 Q
9 S DW="U2;4",DV="NJ11,2",DU="",DLB="PRIMARY PRIOR PAYMENT",DIFLD=218
 S DE(DW)="C9^IBXSC67"
 G RE
...
X11 S:'$D(^DGCR(399,DA,"I2")) Y="@673"
 Q
12 D:$D(DG)>9 F^DIE17,DE S DQ=12,DW="U2;5",DV="NJ11,2",DU="",DLB="SECONDARY PRIOR PAYMENT",DIFLD=219
 S DE(DW)="C12^IBXSC67"
 G RE
...
X14 S:'$D(^DGCR(399,DA,"I3")) Y="@68"
 Q
15 D:$D(DG)>9 F^DIE17,DE S DQ=15,DW="U2;6",DV="NJ11,2",DU="",DLB="TERTIARY PRIOR PAYMENT",DIFLD=220
 S DE(DW)="C15^IBXSC67"
 G RE
