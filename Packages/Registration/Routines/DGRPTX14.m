...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 ...
 I $D(^(.32)) S %Z=^(.32) S %=$P(%Z,U,3) S:%]"" DE(17)=%
 I $D(^(.321)) S %Z=^(.321) S %=$P(%Z,U,2) S:%]"" DE(4)=% S %=$P(%Z,U,3) S:%]"" DE(5)=%
 I $D(^(.322)) S %Z=^(.322) S %=$P(%Z,U,13) S:%]"" DE(6)=%
 I $D(^(.36)) S %Z=^(.36) S %=$P(%Z,U,1) S:%]"" DE(15)=% S %=$P(%Z,U,2) S:%]"" DE(7)=%
 I $D(^(.362)) S %Z=^(.362) S %=$P(%Z,U,12) S:%]"" DE(12)=% S %=$P(%Z,U,13) S:%]"" DE(13)=% S %=$P(%Z,U,14) S:%]"" DE(14)=%
...
X7 S DFN=DA D SV^DGLOCK
 Q
 ;
8 D:$D(DG)>9 F^DIE17,DE S DQ=8,DW=".3;1",DV="SXa",DU="",DLB="SERVICE CONNECTED?",DIFLD=.301
 S DE(DW)="C8^DGRPTX14"
 S DU="Y:YES;N:NO;"
 G RE
...
X12 S DFN=DA D MV^DGLOCK I $D(X) S DFN=DA D EV^DGLOCK
 Q
 ;
13 D:$D(DG)>9 F^DIE17,DE S DQ=13,DW=".362;13",DV="SX",DU="",DLB="RECEIVING HOUSEBOUND BENEFITS?",DIFLD=.36215
...
X13 S DFN=DA D MV^DGLOCK I $D(X) S DFN=DA D EV^DGLOCK
 Q
 ;
14 D:$D(DG)>9 F^DIE17,DE S DQ=14,DW=".362;14",DV="SX",DU="",DLB="RECEIVING A VA PENSION?",DIFLD=.36235
...
C15S S X="" G:DG(DQ)=X C15F1 K DB
 D ^DGRPTX16
C15F1 N X,X1,X2 S DIXR=743 D C15X1(U) K X2 M X2=X D C15X1("O") K X1 M X1=X
 D
...
X15 S DFN=DA D EV^DGLOCK I $D(X) D ECD^DGLOCK1
 Q
 ;
16 D:$D(DG)>9 F^DIE17,DE S DQ=16,D=0 K DE(1) ;361
...
R16 D DE
 ...
 ;
17 S DW=".32;3",DV="*P21'Xa",DU="",DLB="PERIOD OF SERVICE",DIFLD=.323
