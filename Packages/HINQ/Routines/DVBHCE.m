DVBHCE ; GENERATED FROM 'DVBHINQ UPDATE' INPUT TEMPLATE(#960), FILE 2;09/25/12
 D DE G BEGIN
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 I $D(^(.32)) S %Z=^(.32) S %=$P(%Z,U,4) S:%]"" DE(23)=% S %=$P(%Z,U,5) S:%]"" DE(21)=% S %=$P(%Z,U,6) S:%]"" DE(19)=% S %=$P(%Z,U,7) S:%]"" DE(20)=% S %=$P(%Z,U,8) S:%]"" DE(24)=% S %=$P(%Z,U,9) S:%]"" DE(35)=% S %=$P(%Z,U,10) S:%]"" DE(34)=%
 I  S %=$P(%Z,U,11) S:%]"" DE(32)=% S %=$P(%Z,U,12) S:%]"" DE(33)=% S %=$P(%Z,U,13) S:%]"" DE(36)=% S %=$P(%Z,U,16) S:%]"" DE(44)=% S %=$P(%Z,U,19) S:%]"" DE(30)=% S %=$P(%Z,U,20) S:%]"" DE(42)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X32 S %DT="E",%DT(0)=-DT D ^%DT K %DT S X=Y K:Y<1 X I $D(X) S DFN=DA D SER1^DGLOCK I $D(X) K:'$$VALMSE^DGRPMS(DFN,X,0,"MSNTL") X I $D(X) S DGFRDT=X I $D(^DG(43,1)) S SD1=2 D POS^DGINP
 Q
 ;
33 D:$D(DG)>9 F^DIE17,DE S DQ=33,DW=".32;12",DV="RDX",DU="",DLB="NTL-RAD",DIFLD=.3293
 S DE(DW)="C33^DVBHCE",DE(DW,"INDEX")=1
 G RE
...
C33F1 S DIEZRXR(2,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=649 S DIEZRXR(2,DIXR)=""
 Q
X33 S %DT="E",%DT(0)=-DT D ^%DT K %DT S X=Y K:Y<1 X I $D(X) S DFN=DA D SER1^DGLOCK I $D(X) K:'$$VALMSE^DGRPMS(DFN,X,1,"MSNTL") X I $D(X),$D(^DG(43,1)) S SD1=2 D PS^DGINP
 Q
 ;
34 D:$D(DG)>9 F^DIE17,DE S DQ=34,DW=".32;10",DV="P23'X",DU="",DLB="NTL-Bran. Ser.",DIFLD=.3291
...
C34S S X="" G:DG(DQ)=X C34F1 K DB
 ...
 S X=DG(DQ),DIC=DIE
 ;
C34F1 N X,X1,X2 S DIXR=409 D C34X1(U) K X2 M X2=X D C34X1("O") K X1 M X1=X
 D
 . N DIEXARR M DIEXARR=X S DIEZCOND=1
 . S X=X2(1)=""
 . S DIEZCOND=$G(X) K X M X=DIEXARR Q:'DIEZCOND
...
C34X1(DION) K X
 ...
 Q
C34F2 Q
X34 S DFN=DA K:X=$O(^DIC(23,"B","B.E.C.","")) X I $D(X) D SER1^DGLOCK S DGCOMBR=$G(Y) Q
 Q
 ;
35 D:$D(DG)>9 F^DIE17,DE S DQ=35,DW=".32;9",DV="RP25'X",DU="",DLB="NTL-Char. Ser.",DIFLD=.329
 S DE(DW)="C35^DVBHCE"
 S DU="DIC(25,"
 G RE
...
C35S S X="" G:DG(DQ)=X C35F1 K DB
 ...
 D EVENT^IVMPLOG(DA)
C35F1 Q
X35 S DFN=DA D SER1^DGLOCK
 Q
 ;
36 D:$D(DG)>9 F^DIE17,DE S DQ=36,DW=".32;13",DV="FX",DU="",DLB="NTL-Ser. Num.",DIFLD=.3294
 S DE(DW)="C36^DVBHCE"
 G RE
...
C36S S X="" G:DG(DQ)=X C36F1 K DB
 ...
 D EVENT^IVMPLOG(DA)
C36F1 Q
X36 S DFN=DA D SER1^DGLOCK I $D(X) S:X?1"SS".E L=$S($D(^DPT(DA,0)):$P(^(0),U,9),1:X) W:X?1"SS".E "  ",L S:X?1"SS".E X=L K:$L(X)>15!($L(X)<1)!'(X?.N) X
 I $D(X),X'?.ANP K X
 Q
 ;
37 S DQ=38 ;@31
...
X39 I '$D(^DPT(D0,.32)) W !!,*7,?17,DVBON,"No NTLast episode can't edit NNTLast   <RET>",DVBOFF X DVBLIT1 R DVBZ:DTIME K DVBZ S Y="@33"
 Q
40 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=40 D X40 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X40 I $P(^DPT(D0,.32),U,19)'="Y" W !!,*7,?17,DVBON,"No NTLast episode can't edit NNTLast   <RET>.",DVBOFF X DVBLIT1 R DVBZ:DTIME K DVBZ S Y="@33"
 Q
41 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=41 D X41 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X41 S DVBSCR=1 D ^DVBHS4 W !,?34,DVBON,"[NNTLAST]",DVBOFF X DVBLIT1
 Q
42 D:$D(DG)>9 F^DIE17,DE S DQ=42,DW=".32;20",DV="RSX",DU="",DLB="Service NNTL Episode",DIFLD=.32945
 S DE(DW)="C42^DVBHCE"
 S DU="Y:YES;N:NO;"
 G RE
...
C42S S X="" G:DG(DQ)=X C42F1 K DB
 D ^DVBHCE2
C42F1 Q
X42 S DFN=DA D SV^DGLOCK I "N"'[$G(X),$D(^DPT(DFN,.32)),$P(^(.32),U,19)'="Y" W !?4,*7,"Other Periods of service are not indicated...NO EDITING!" K X
 Q
 ;
43 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=43 D X43 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X43 I $P(^DPT(D0,.32),U,20)'="Y" S Y="@33"
 Q
44 D:$D(DG)>9 F^DIE17,DE S DQ=44,DW=".32;16",DV="RDX",DU="",DLB="NNTL-EOD",DIFLD=.3297
 S DE(DW)="C44^DVBHCE",DE(DW,"INDEX")=1
 G RE
C44 G C44S:$D(DE(44))[0 K DB
 D ^DVBHCE3
C44S S X="" G:DG(DQ)=X C44F1 K DB
 D ^DVBHCE4
C44F1 S DIEZRXR(2,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=663 S DIEZRXR(2,DIXR)=""
 Q
X44 S %DT="E",%DT(0)=-DT D ^%DT K %DT S X=Y K:Y<1 X I $D(X) S DFN=DA D SER2^DGLOCK I $D(X) K:'$$VALMSE^DGRPMS(DFN,X,0,"MSNNTL") X I $D(X) S DGFRDT=X I $D(^DG(43,1)) S SD1=3 D POS^DGINP
 Q
 ;
45 D:$D(DG)>9 F^DIE17 G ^DVBHCE5
