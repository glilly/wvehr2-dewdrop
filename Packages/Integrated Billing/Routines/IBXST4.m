...
DE S DIE="^DGCR(399,",DIC=DIE,DP=399,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGCR(399,DA,""))=""
 I $D(^("S")) S %Z=^("S") S %=$P(%Z,U,9) S:%]"" DE(4)=% S %=$P(%Z,U,14) S:%]"" DE(1)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
BEGIN S DNM="IBXST4",DQ=1
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW="S;14",DV="D",DU="",DLB="DATE LAST PRINTED",DIFLD=14
 S DE(DW)="C1^IBXST4"
 S X=DT
 S Y=X
...
X2 S Y="@99"
 Q
3 S DQ=4 ;@902
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW="S;9",DV="FOX",DU="",DLB="AUTHORIZE BILL GENERATION?",DIFLD=9
