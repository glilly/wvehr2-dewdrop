...
2 S DW="0;1",DV="MRNJ3,0",DU="",DLB="LINE ITEM NUMBER",DIFLD=.01
 S DE(DW)="C2^PRCST41",DE(DW,"INDEX")=1
 S Y="1"
 G Y
...
4 S DW="0;2",DV="RNJ9,2",DU="",DLB="QUANTITY",DIFLD=2
 S DE(DW)="C4^PRCST41",DE(DW,"INDEX")=1
 G RE
...
C4F1 S DIEZRXR(410.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=653 S DIEZRXR(410.02,DIXR)=""
 Q
...
5 D:$D(DG)>9 F^DIE17,DE S DQ=5,DW="0;3",DV="RP420.5'",DU="",DLB="UNIT OF PURCHASE",DIFLD=3
 S DE(DW)="C5^PRCST41",DE(DW,"INDEX")=1
 S DU="PRCD(420.5,"
 G RE
...
C5F1 S DIEZRXR(410.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 ...
 Q
X5 Q
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW="0;6",DV="FX",DU="",DLB="STOCK NUMBER",DIFLD=6
 S DE(DW)="C6^PRCST41",DE(DW,"INDEX")=1
 G RE
...
X6 K:$L(X)>24!($L(X)<1)!(X'?.ANP) X
 ...
 Q
 ;
7 D:$D(DG)>9 F^DIE17,DE S DQ=7,DW="0;7",DV="RNJ10,2X",DU="",DLB="EST. ITEM (UNIT) COST",DIFLD=7
 S DE(DW)="C7^PRCST41",DE(DW,"INDEX")=1
 G RE
...
C7S S X="" G:DG(DQ)=X C7F1 K DB
 S X=DG(DQ),DIC=DIE
 X "S E=0,E(1)="""" S:'$D(^PRCS(410,DA(1),4)) ^(4)="""" F E(0)=1:1 S E=$O(^PRCS(410,DA(1),""IT"",E)) S:E?1N.N E(1)=E(1)+($P(^(E,0),U,2)*$P(^(0),U,7)) I E'?1N.N X ^DD(410.02,2,1,1,1.4) K E Q"
C7F1 S DIEZRXR(410.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=653 S DIEZRXR(410.02,DIXR)=""
 Q
...
9 D:$D(DG)>9 F^DIE17,DE S DQ=9,DW="0;4",DV="RFX",DU="",DLB="BOC",DIFLD=4
 S DE(DW)="C9^PRCST41",DE(DW,"INDEX")=1
...
14 D:$D(DG)>9 F^DIE17,DE S DQ=14,D=0 K DE(1) ;12
 S DIFLD=12,DGO="^PRCST43",DC="2^410.212I^2^",DV="410.212MNJ2,0",DW="0;1",DOW="DELIVERY SCHEDULE",DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
