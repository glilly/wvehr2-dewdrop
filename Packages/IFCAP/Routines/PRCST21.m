...
2 S DW="0;1",DV="MRNJ3,0",DU="",DLB="LINE ITEM NUMBER",DIFLD=.01
 S DE(DW)="C2^PRCST21",DE(DW,"INDEX")=1
 S Y="1"
 G Y
...
C2F1 S DIEZRXR(410.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
...
5 D:$D(DG)>9 F^DIE17,DE S DQ=5,DW="0;5",DV="FX",DU="",DLB="ITEM MASTER FILE NO.",DIFLD=5
 S DE(DW)="C5^PRCST21",DE(DW,"INDEX")=1
 G RE
...
C5F1 S DIEZRXR(410.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=653 S DIEZRXR(410.02,DIXR)=""
 Q
...
9 D:$D(DG)>9 F^DIE17,DE S DQ=9,DW="0;4",DV="RFX",DU="",DLB="BOC",DIFLD=4
 S DE(DW)="C9^PRCST21",DE(DW,"INDEX")=1
 G RE
...
C9F1 S DIEZRXR(410.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=653 S DIEZRXR(410.02,DIXR)=""
 Q
...
11 D:$D(DG)>9 F^DIE17,DE S DQ=11,DW="0;2",DV="RNJ9,2",DU="",DLB="QUANTITY",DIFLD=2
 S DE(DW)="C11^PRCST21",DE(DW,"INDEX")=1
 G RE
...
C11F1 S DIEZRXR(410.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=653 S DIEZRXR(410.02,DIXR)=""
 Q
...
15 S DW="0;4",DV="RFX",DU="",DLB="BOC",DIFLD=4
 S DE(DW)="C15^PRCST21",DE(DW,"INDEX")=1
 G RE
...
C15S S X="" G:DG(DQ)=X C15F1 K DB
 S X=DG(DQ),DIC=DIE
 S ^PRCS(410,"AD",$E(X,1,30),DA(1))=""
C15F1 S DIEZRXR(410.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=653 S DIEZRXR(410.02,DIXR)=""
 Q
...
18 D:$D(DG)>9 F^DIE17,DE S DQ=18,DW="0;2",DV="RNJ9,2",DU="",DLB="QUANTITY",DIFLD=2
 S DE(DW)="C18^PRCST21",DE(DW,"INDEX")=1
 G RE
...
C18S S X="" G:DG(DQ)=X C18F1 K DB
 S X=DG(DQ),DIC=DIE
 X "S E=0,E(1)="""" S:'$D(^PRCS(410,DA(1),4)) ^(4)="""" F E(0)=1:1 S E=$O(^PRCS(410,DA(1),""IT"",E)) S:E?1N.N E(1)=E(1)+($P(^(E,0),U,2)*$P(^(0),U,7)) I E'?1N.N X ^DD(410.02,2,1,1,1.4) K E Q"
C18F1 S DIEZRXR(410.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=653 S DIEZRXR(410.02,DIXR)=""
 Q
...
19 D:$D(DG)>9 F^DIE17,DE S DQ=19,DW="0;3",DV="RP420.5'",DU="",DLB="UNIT OF PURCHASE",DIFLD=3
 S DE(DW)="C19^PRCST21",DE(DW,"INDEX")=1
 S DU="PRCD(420.5,"
 G RE
...
C19F1 S DIEZRXR(410.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 ...
 Q
X19 Q
20 D:$D(DG)>9 F^DIE17,DE S DQ=20,DW="0;6",DV="FX",DU="",DLB="STOCK NUMBER",DIFLD=6
 S DE(DW)="C20^PRCST21",DE(DW,"INDEX")=1
 G RE
...
X20 K:$L(X)>24!($L(X)<1)!(X'?.ANP) X
 ...
 Q
 ;
21 D:$D(DG)>9 F^DIE17,DE S DQ=21,DW="0;7",DV="RNJ10,2X",DU="",DLB="EST. ITEM (UNIT) COST",DIFLD=7
 S DE(DW)="C21^PRCST21",DE(DW,"INDEX")=1
 G RE
