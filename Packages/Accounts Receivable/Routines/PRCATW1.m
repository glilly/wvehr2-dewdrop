PRCATW1 ; GENERATED FROM 'PRCAC TR LIST' PRINT TEMPLATE (#408) ; 09/19/10 ; (FILE 433, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 D N:$X>37 Q:'DN  W ?37 S Y=$P(X,U,2) S Y=$S(Y="":Y,$D(^PRCA(430.3,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,30)
 D N:$X>59 Q:'DN  W ?59 S Y=$P(X,U,7) W:Y]"" $S($D(DXS(1,Y)):DXS(1,Y),1:Y)
 D N:$X>2 Q:'DN  W ?2 W "PROCESSED BY:"
 S X=$G(^PRCA(433,D0,0)) W ?17 S Y=$P(X,U,9) S Y=$S(Y="":Y,$D(^VA(200,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,35)
