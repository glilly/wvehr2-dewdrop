PRCST6 ; GENERATED FROM 'PRCS REQUESTS FOR APPROVAL' PRINT TEMPLATE (#304) ; 09/19/10 ; (FILE 410, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 W ?26 S Y=$P(X,U,4) S Y=$S(Y="":Y,$D(^PRCS(410.5,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,30)
 S X=$G(^PRCS(410,D0,7)) D N:$X>2 Q:'DN  W ?2 S Y=$P(X,U,1) S Y=$S(Y="":Y,$D(^VA(200,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,35)
