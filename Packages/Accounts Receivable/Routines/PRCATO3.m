PRCATO3 ; GENERATED FROM 'PRCAP COST' PRINT TEMPLATE (#400) ; 09/19/10 ; (FILE 433, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 S X=$G(^PRCA(433,D0,0)) D N:$X>12 Q:'DN  W ?12 S Y=$P(X,U,2) S Y=$S(Y="":Y,$D(^PRCA(430,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,12)
 D N:$X>34 Q:'DN  W ?34 W "TRANSACTION DATE: "
 S X=$G(^PRCA(433,D0,1)) D N:$X>54 Q:'DN  W ?54 S Y=$P(X,U,1) D DT
 D N:$X>0 Q:'DN  W ?0 W "TYPE: "
 D N:$X>11 Q:'DN  W ?11 S Y=$P(X,U,2) S Y=$S(Y="":Y,$D(^PRCA(430.3,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,30)
