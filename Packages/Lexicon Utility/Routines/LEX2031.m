LEX2031 ;ISL/KER - Environment Check/Pre/Post Install ; 10/01/2004
 ...
 ;
ENV ; LEX*2.0*31 Environment Check
 ;
 ;   General
 ;
 N LEXBUILD,LEXIGHF,LEXREQP,LEXLREV,LEXG,LEXE
 W !," Code Set Quarterly Update w/FY 2005 ICD-9-CM Changes",!
 D IMP S U="^"
 ;     No user
...
ABRT ;     Abort  Failed Environment Check, KILL the distribution
 D:$D(LEXE) ED S XPDABORT=1,XPDQUIT=1,XPDQUIT("LEX*2.0*31")=1
...
CPD(X) ;  Check Current Patched Data is installed
 ...
 S:'$D(^LEX(757.1,"B",182402,259569)) INS=0
 S:$D(^LEX(757.02,"ADCODE","A4646 ",2720)) INS=0
 ;    Check Last ICD Diagnosis Set/Kill
 S:'$D(^ICD9("D","WORKING",13955)) INS=0
 S:$D(^ICD9("D","WITHDRAWAL",1087)) INS=0
 ;    Check Modified Text
 S:$G(^ICD9(7766,1))'="OTHER SPECIFIED PULMONARY TUBERCULOSIS, UNSPECIFIED EXAMINATION" INS=0
 ;    Check Last ICD Procedures Set/Kill
 S:'$D(^ICD0("E","VERTEBROPLASTY",4221)) INS=0
 S:$D(^ICD0("D","TYPE",2942)) INS=0
 ;    Check Last CPT/HCPCS Procedures Set
 S:'$D(^ICPT("ACT","A9525 ",0,3040401,107271,2)) INS=0
 S X=+($G(INS))
 Q X
 ;
 ; Error messages
...
ED ;   Error Display
 ...
 ;
IMP ;   Import names
 S LEXLREV=16,LEXREQP="ICD*18.0*12;ICPT*6.0*19;LEX*2.0*30",LEXBUILD="LEX*2.0*31"
 S LEXIGHF="LEX20_31.GBL"
 Q
