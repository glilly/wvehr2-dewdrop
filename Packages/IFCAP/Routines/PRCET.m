PRCET ; GENERATED FROM 'PRCE DAILY RECORD EDIT' PRINT TEMPLATE (#344) ; 09/19/10 ; (FILE 424.1, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 W ?36 S Y=$P(X,U,2) S Y=$S(Y="":Y,$D(^PRC(424,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,15)
 ...
 S X=$G(^PRC(424.1,D0,0)) W ?24 S Y=$P(X,U,10) S Y=$S(Y="":Y,$D(^VA(200,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,35)
