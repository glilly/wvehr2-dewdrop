...
ABRT ;     Abort  Failed Environment Check, KILL the distribution
 D:$D(LEXE) ED S XPDABORT=1,XPDQUIT=1,XPDQUIT("LEX*2.0*34")=1
...
CPD(X) ;  Check Current Patched Data is installed
 ...
 S:'$D(^ICPT("F","V2702",107957)) INS=0
 S:$D(^ICPT("C","YOUNG",99294,3)) INS=0
 ;    Check Last CPT Modifiers Set
 S:'$D(^DIC(81.3,493,61,0)) INS=0
 ;    V2
 S:+($P($G(^LEXC(757.91,21036,0)),"^",7))>0 INS=0
...
ED ;   Error Display
 ...
 ;
IMP ;   Import names
 S LEXLREV=34,LEXREQP="ICPT*6.0*19;LEX*2.0*31",LEXBUILD="LEX*2.0*34"
 S LEXIGHF="LEX_2_34.GBL"
 Q
