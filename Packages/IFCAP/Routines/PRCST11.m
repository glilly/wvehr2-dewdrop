...
3 S DW="0;1",DV="MRNJ3,0",DU="",DLB="LINE ITEM NUMBER",DIFLD=.01
 S DE(DW)="C3^PRCST11",DE(DW,"INDEX")=1
 S Y="1"
 G Y
...
C3F1 S DIEZRXR(410.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=653 S DIEZRXR(410.02,DIXR)=""
 Q
...
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW="0;5",DV="FX",DU="",DLB="ITEM MASTER FILE NO.",DIFLD=5
 S DE(DW)="C6^PRCST11",DE(DW,"INDEX")=1
 G RE
...
C6F1 S DIEZRXR(410.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=653 S DIEZRXR(410.02,DIXR)=""
 Q
...
10 D:$D(DG)>9 F^DIE17,DE S DQ=10,DW="0;4",DV="RFX",DU="",DLB="BOC",DIFLD=4
 S DE(DW)="C10^PRCST11",DE(DW,"INDEX")=1
 G RE
...
C10F1 S DIEZRXR(410.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=653 S DIEZRXR(410.02,DIXR)=""
 Q
...
12 D:$D(DG)>9 F^DIE17,DE S DQ=12,DW="0;2",DV="RNJ9,2",DU="",DLB="QUANTITY",DIFLD=2
 S DE(DW)="C12^PRCST11",DE(DW,"INDEX")=1
 G RE
...
C12F1 S DIEZRXR(410.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=653 S DIEZRXR(410.02,DIXR)=""
 Q
...
16 S DW="0;4",DV="RFX",DU="",DLB="BOC",DIFLD=4
 S DE(DW)="C16^PRCST11",DE(DW,"INDEX")=1
 G RE
...
C16S S X="" G:DG(DQ)=X C16F1 K DB
 S X=DG(DQ),DIC=DIE
 S ^PRCS(410,"AD",$E(X,1,30),DA(1))=""
C16F1 S DIEZRXR(410.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=653 S DIEZRXR(410.02,DIXR)=""
 Q
...
19 D:$D(DG)>9 F^DIE17,DE S DQ=19,DW="0;2",DV="RNJ9,2",DU="",DLB="QUANTITY",DIFLD=2
 S DE(DW)="C19^PRCST11",DE(DW,"INDEX")=1
 G RE
...
C19S S X="" G:DG(DQ)=X C19F1 K DB
 S X=DG(DQ),DIC=DIE
 X "S E=0,E(1)="""" S:'$D(^PRCS(410,DA(1),4)) ^(4)="""" F E(0)=1:1 S E=$O(^PRCS(410,DA(1),""IT"",E)) S:E?1N.N E(1)=E(1)+($P(^(E,0),U,2)*$P(^(0),U,7)) I E'?1N.N X ^DD(410.02,2,1,1,1.4) K E Q"
C19F1 S DIEZRXR(410.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=653 S DIEZRXR(410.02,DIXR)=""
 Q
...
20 D:$D(DG)>9 F^DIE17,DE S DQ=20,DW="0;3",DV="RP420.5'",DU="",DLB="UNIT OF PURCHASE",DIFLD=3
 S DE(DW)="C20^PRCST11",DE(DW,"INDEX")=1
 S DU="PRCD(420.5,"
 G RE
C20 G C20S:$D(DE(20))[0 K DB
C20S S X="" G:DG(DQ)=X C20F1 K DB
C20F1 S DIEZRXR(410.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=653 S DIEZRXR(410.02,DIXR)=""
 Q
X20 Q
21 D:$D(DG)>9 F^DIE17,DE S DQ=21,DW="0;6",DV="FX",DU="",DLB="STOCK NUMBER",DIFLD=6
 S DE(DW)="C21^PRCST11",DE(DW,"INDEX")=1
 G RE
C21 G C21S:$D(DE(21))[0 K DB
C21S S X="" G:DG(DQ)=X C21F1 K DB
C21F1 S DIEZRXR(410.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=653 S DIEZRXR(410.02,DIXR)=""
 Q
X21 K:$L(X)>24!($L(X)<1)!(X'?.ANP) X
 ...
 Q
 ;
22 D:$D(DG)>9 F^DIE17,DE S DQ=22,DW="0;7",DV="RNJ10,2X",DU="",DLB="EST. ITEM (UNIT) COST",DIFLD=7
 S DE(DW)="C22^PRCST11",DE(DW,"INDEX")=1
 G RE
C22 G C22S:$D(DE(22))[0 K DB
 D ^PRCST16
C22S S X="" G:DG(DQ)=X C22F1 K DB
 D ^PRCST17
C22F1 S DIEZRXR(410.02,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 F DIXR=653 S DIEZRXR(410.02,DIXR)=""
 Q
