...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 I $D(^(.3)) S %Z=^(.3) S %=$P(%Z,U,2) S:%]"" DE(26)=% S %=$P(%Z,U,5) S:%]"" DE(1)=% S %=$P(%Z,U,14) S:%]"" DE(27)=%
 I $D(^(.32)) S %Z=^(.32) S %=$P(%Z,U,2) S:%]"" DE(8)=%
 I $D(^(.321)) S %Z=^(.321) S %=$P(%Z,U,1) S:%]"" DE(14)=%
 I $D(^(.362)) S %Z=^(.362) S %=$P(%Z,U,8) S:%]"" DE(53)=% S %=$P(%Z,U,15) S:%]"" DE(41)=% S %=$P(%Z,U,18) S:%]"" DE(47)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X26 S DFN=DA D EV^DGLOCK Q:'$D(X)  K:+X'=X!(X>100)!(X<0)!(X?.E1"."1N.N) X I $D(X),$D(^DPT(DA,.3)),$P(^(.3),U,1)'="Y" W !?4,*7,"Only applies to service-connected applicants." K X
 Q
 ;
27 D:$D(DG)>9 F^DIE17,DE S DQ=27,DW=".3;14",DV="DX",DU="",DLB="EFF. DATE COMBINED SC% EVAL.",DIFLD=.3014
 S X=$G(DVBEFF)
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
X27 S %DT="P" D ^%DT S X=Y K:Y<1!(Y>DT) X
 Q
 ;
28 S DQ=29 ;@46
29 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=29 D X29 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X29 S JP=$O(DVBDX(JP)) I 'JP S Y="@50"
 Q
30 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=30 D X30 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X30 S JPP=+$P(DVBDX(JP),U,2) I JPP'>0 S Y="@46"
 Q
31 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=31 D X31 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X31 I '$D(^DIC(31,JPP)) D CHKDIS^DVBHS3 S Y="@46"
 Q
32 S D=0 K DE(1) ;.3721
 S DIFLD=.3721,DGO="^DVBHCE27",DC="6^2.04P^.372^",DV="2.04MP31'X",DW="0;1",DOW="RATED DISABILITIES (VA)",DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
 S DU="DIC(31,"
 G RE:D I $D(DSC(2.04))#2,$P(DSC(2.04),"I $D(^UTILITY(",1)="" X DSC(2.04) S D=$O(^(0)) S:D="" D=-1 G M32
 S D=$S($D(^DPT(DA,.372,0)):$P(^(0),U,3,4),$O(^(0))'="":$O(^(0)),1:-1)
M32 I D>0 S DC=DC_D I $D(^DPT(DA,.372,+D,0)) S DE(32)=$P(^(0),U,1)
