...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 I $D(^(.11)) S %Z=^(.11) S %=$P(%Z,U,4) S:%]"" DE(2)=% S %=$P(%Z,U,5) S:%]"" DE(4)=% S %=$P(%Z,U,7) S:%]"" DE(5)=% S %=$P(%Z,U,12) S:%]"" DE(1)=% S %=$P(%Z,U,16) S:%]"" DE(8)=%
 I $D(^(.13)) S %Z=^(.13) S %=$P(%Z,U,1) S:%]"" DE(6)=% S %=$P(%Z,U,2) S:%]"" DE(7)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW=".11;4",DV="Fa",DU="",DLB="CITY",DIFLD=.114
 S DE(DW)="C2^DGRPTX5",DE(DW,"INDEX")=1
 G RE
...
X3 S:'$$KEY^DGREGDD1(DUZ,DA) Y=.131
 Q
...
X5 S Z0=$S($D(^DPT(D0,.11)):+$P(^(.11),"^",5),1:0) K:'Z0 X Q:'Z0!'$D(^DIC(5,Z0,1,0))  S DIC="^DIC(5,Z0,1,",DIC(0)="QEM" D ^DIC S X=+Y K:Y'>0 X K Z0,DIC
 Q
 ;
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW=".13;1",DV="Fa",DU="",DLB="PHONE NUMBER [RESIDENCE]",DIFLD=.131
 S DE(DW)="C6^DGRPTX5"
 G RE
...
X6 K:$L(X)>20!($L(X)<4) X
 ...
 Q
 ;
7 D:$D(DG)>9 F^DIE17,DE S DQ=7,DW=".13;2",DV="Fa",DU="",DLB="PHONE NUMBER [WORK]",DIFLD=.132
 S DE(DW)="C7^DGRPTX5"
 G RE
...
X7 K:$L(X)>20!($L(X)<4) X
 ...
 Q
 ;
8 D:$D(DG)>9 F^DIE17,DE S DQ=8,DW=".11;16",DV="*S",DU="",DLB="BAD ADDRESS INDICATOR",DIFLD=.121
 S DE(DW)="C8^DGRPTX5",DE(DW,"INDEX")=1
...
C8S S X="" G:DG(DQ)=X C8F1 K DB
 D ^DGRPTX12
C8F1 N X,X1,X2 S DIXR=728 D C8X1(U) K X2 M X2=X D C8X1("O") K X1 M X1=X
 D
