ONCOES  ;Hines OIFO/GWB - ONCOLOGY PATIENT (160) computed fields ;08/10/00
        ;;2.11;ONCOLOGY;**1,5,6,11,15,16,18,26,27,33,34,35,36,44,46,49**;Mar 07, 1995;Build 38
        ;
...
DOB1    ;date of birth returned in X in File Manager format
        ...
        G END
        ;
MS      ;MARITAL STATUS (160,14)
        I $P(^ONCO(160,D0,0),U,1)["LRT(67," Q
        S DPTIEN=$P(^ONCO(160,D0,0),";",1)
        S X=$$GET1^DIQ(2,DPTIEN,.05,"E") ;MARITAL STATUS
        K DPTIEN
        Q
        ;
REL     ;RELIGION (160,13)
        ...
        I $P(^ONCO(160,D0,0),U,1)["LRT(67," Q
        S DPTIEN=$P(^ONCO(160,D0,0),";",1)
        S X=$$GET1^DIQ(2,DPTIEN,.08,"E") ;RELIGIOUS PREFERENCE
        K DPTIEN
        Q
        ;
...
SA3     ;
        ...
        G END
NOTE    ;NONE OF THE COMPUTED FIELDS BELOW USING ZIP CODE FILE CAN BE USED SO LONG AS MAS DOES NOT USE THE ZIP CODE FILE;THE ZIP CODE PICKED WILL BE THE FIRST IF THE LIST IF MORE THAN ONE SELECTION
...
ZIP1    I $D(^ONCO(160,D0,0)) D SETUP1 S X=$S($D(@ONCOX1):$P(@ONCOX1,U,6),1:"")
        ...
        G END
CT      ;Get county from ZIP CODE (#5.11)
        ...
        G END
ZCTY    ;Get county from ZIP CODE (#5.11)
...
PH      ;
        ...
        G END
        ;
EMP     ;EMPLOYMENT STATUS (#160,47)
        I $P(^ONCO(160,D0,0),U,1)["LRT(67," Q
        S DPTIEN=$P(^ONCO(160,D0,0),";",1)
        S X=$$GET1^DIQ(2,DPTIEN,.31115,"E") ;EMPLOYMENT STATUS
        K DPTIEN
        Q
        ;
ENVIRON ;Environmental exposure defaults
        I $P(^ONCO(160,D0,0),U,1)["LRT(67," D  Q
        .S (AOE,IRE,PGS,SS,LSI,VSI,GSI,PSI,YSI)=""
        S DPTIEN=$P(^ONCO(160,D0,0),";",1)
        S AOE=$$GET1^DIQ(2,DPTIEN,.32102,"E")  ;AGENT ORANGE EXPOS. INDICATED?
        S IRE=$$GET1^DIQ(2,DPTIEN,.32103,"E")  ;RADIATION EXPOSURE INDICATED?
        S PGS=$$GET1^DIQ(2,DPTIEN,.32201,"E")  ;PERSIAN GULF SERVICE?
        S SS=$$GET1^DIQ(2,DPTIEN,.322016,"E")  ;SOMALIA SERVICE INDICATED?
        S LSI=$$GET1^DIQ(2,DPTIEN,.3221,"E")   ;LEBANON SERVICE INDICATED?
        S VSI=$$GET1^DIQ(2,DPTIEN,.32101,"E")  ;VIETNAM SERVICE INDICATED?
        S GSI=$$GET1^DIQ(2,DPTIEN,.3224,"E")   ;GRENADA SERVICE INDICATED?
        S PSI=$$GET1^DIQ(2,DPTIEN,.3227,"E")   ;PANAMA SERVICE INDICATED?
        S YSI=$$GET1^DIQ(2,DPTIEN,.322019,"E") ;YUGOSLAVIA SERVICE INDICATED?
        K DPTIEN
        Q
        ;
