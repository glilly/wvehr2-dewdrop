ENPLPC ; GENERATED FROM 'ENPLP005' PRINT TEMPLATE (#157) ; 09/19/10 ; (FILE 6925, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 S Y=$P(X,U,2) S Y=$S(Y="":Y,$D(^OFM(7336.9,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,40)
 ...
 G A1R
A1 ;
 S X=$G(^ENG("PROJ",D0,17,D1,0)) S DIWL=23,DIWR=78 D ^DIWP
 Q
A1R ;
 D 0^DIWW
 D ^DIWW
 ...
 G B1R
B1 ;
 S X=$G(^ENG("PROJ",D0,26,D1,0)) S DIWL=25,DIWR=78 D ^DIWP
 Q
B1R ;
 D 0^DIWW
 D ^DIWW
 ...
 G C1R
C1 ;
 S X=$G(^ENG("PROJ",D0,27,D1,0)) S DIWL=12,DIWR=78 D ^DIWP
 Q
C1R ;
 D 0^DIWW
 D ^DIWW K Y K DIWF
