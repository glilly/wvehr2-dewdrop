...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 ...
 I $D(^(.362)) S %Z=^(.362) S %=$P(%Z,U,12) S:%]"" DE(2)=% S %=$P(%Z,U,13) S:%]"" DE(3)=% S %=$P(%Z,U,14) S:%]"" DE(1)=%
 I $D(^("TYPE")) S %Z=^("TYPE") S %=$P(%Z,U,1) S:%]"" DE(5)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X1 S DFN=DA D MV^DGLOCK
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW=".362;12",DV="SX",DU="",DLB="RECEIVING A&A BENEFITS?",DIFLD=.36205
 S DE(DW)="C2^A1CKC5"
 S DU="Y:YES;N:NO;U:UNKNOWN;"
 S X=$S(AA="Y":"Y",1:"N")
 S Y=X
...
X2 S DFN=DA D MV^DGLOCK I $D(X) S DFN=DA D EV^DGLOCK
 Q
 ;
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW=".362;13",DV="SX",DU="",DLB="RECEIVING HOUSEBOUND BENEFITS?",DIFLD=.36215
 S DE(DW)="C3^A1CKC5"
 S DU="Y:YES;N:NO;U:UNKNOWN;"
 S X=$S(HB="Y":"Y",1:"N")
 S Y=X
...
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW=".36;1",DV="*P8'Xa",DU="",DLB="PRIMARY ELIGIBILITY CODE",DIFLD=.361
 S DE(DW)="C4^A1CKC5",DE(DW,"INDEX")=1
 S DU="DIC(8,"
 S X=ELIG
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
C4F1 N X,X1,X2 S DIXR=743 D C4X1(U) K X2 M X2=X D C4X1("O") K X1 M X1=X
 D
...
X4 S DFN=DA D EV^DGLOCK I $D(X) D ECD^DGLOCK1
 Q
 ;
5 D:$D(DG)>9 F^DIE17,DE S DQ=5,DW="TYPE;1",DV="P391'a",DU="",DLB="TYPE",DIFLD=391
 S DE(DW)="C5^A1CKC5",DE(DW,"INDEX")=1
 S DU="DG(391,"
 S X=DZT2
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
C5F1 N X,X1,X2 S DIXR=664 D C5X1(U) K X2 M X2=X D C5X1("O") K X1 M X1=X
 I $G(X(1))]"" D
 . K ^DPT("APTYPE",X,DA)
