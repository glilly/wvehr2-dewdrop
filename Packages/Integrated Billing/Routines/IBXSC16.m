...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=2,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 I $D(^(.11)) S %Z=^(.11) S %=$P(%Z,U,2) S:%]"" DE(1)=% S %=$P(%Z,U,3) S:%]"" DE(3)=% S %=$P(%Z,U,4) S:%]"" DE(4)=% S %=$P(%Z,U,5) S:%]"" DE(5)=% S %=$P(%Z,U,7) S:%]"" DE(7)=% S %=$P(%Z,U,12) S:%]"" DE(6)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW=".11;4",DV="Fa",DU="",DLB="CITY",DIFLD=.114
 S DE(DW)="C4^IBXSC16",DE(DW,"INDEX")=1
 G RE
...
C5S S X="" G:DG(DQ)=X C5F1 K DB
 D ^IBXSC18
C5F1 N X,X1,X2 S DIXR=235 D C5X1(U) K X2 M X2=X D C5X1("O") K X1 M X1=X
 D
...
C5X1(DION) K X
 ...
 Q
C5F2 Q
X5 S DIC("S")="I $P(^DIC(5,Y,0),U,6)=1" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
...
C6S S X="" G:DG(DQ)=X C6F1 K DB
 D ^IBXSC110
C6F1 N X,X1,X2 S DIXR=185 D C6X1(U) K X2 M X2=X D C6X1("O") K X1 M X1=X
 D
...
C6X1(DION) K X
 ...
 S X=$G(X(1))
 Q
C6F2 S DIXR=231 D C6X2(U) K X2 M X2=X D C6X2("O") K X1 M X1=X
 D
...
X6 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>20!($L(X)<5) X I $D(X) D ZIPIN^VAFADDR
 ...
 Q
 ;
7 D:$D(DG)>9 F^DIE17,DE S DQ=7,DW=".11;7",DV="NJ3,0XOa",DU="",DLB="COUNTY",DIFLD=.117
 S DQ(7,2)="S Y(0)=Y Q:Y']""""  S Z0=$S($D(^DPT(D0,.11)):+$P(^(.11),""^"",5),1:"""") Q:'Z0  S Y=$P($S($D(^DIC(5,Z0,1,Y,0)):^(0),1:""""),""^"",3)"
 S DE(DW)="C7^IBXSC16"
 G RE
...
C7S S X="" G:DG(DQ)=X C7F1 K DB
 D ^IBXSC112
C7F1 Q
X7 S Z0=$S($D(^DPT(D0,.11)):+$P(^(.11),"^",5),1:0) K:'Z0 X Q:'Z0!'$D(^DIC(5,Z0,1,0))  S DIC="^DIC(5,Z0,1,",DIC(0)="QEM" D ^DIC S X=+Y K:Y'>0 X K Z0,DIC
 Q
 ;
