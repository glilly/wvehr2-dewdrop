YSCPAM ; GENERATED FROM 'YSSR PT INQ PRINT' PRINT TEMPLATE (#564) ; 09/19/10 ; (FILE 615.2, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 I $D(DXS)<9 M DXS=^DIPT(564,"DXS")
 S I(0)="^YS(615.2,",J(0)=615.2
 W ?0 D PARSE^YSSRU K DIP K:DN Y
 D N:$X>0 Q:'DN  W ?0 W "EPISODE"
 D N:$X>0 Q:'DN  W ?0 W "-------"
 D N:$X>0 Q:'DN  W ?0 W "ORDERED BY: "
 S X=$G(^YS(615.2,D0,25)) W ?14 S Y=$P(X,U,1) S Y=$S(Y="":Y,$D(^VA(200,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,35)
