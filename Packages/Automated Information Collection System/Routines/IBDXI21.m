...
DE S DIE="^IBE(357.2,D0,2,",DIC=DIE,DP=357.22,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$O(^IBE(357.2,D0,2,DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,1) S:%]"" DE(4)=% S %=$P(%Z,U,2) S:%]"" DE(25)=%,DE(28)=%,DE(30)=%,DE(39)=%,DE(42)=% S %=$P(%Z,U,3) S:%]"" DE(37)=% S %=$P(%Z,U,4) S:%]"" DE(8)=% S %=$P(%Z,U,5) S:%]"" DE(34)=% S %=$P(%Z,U,6) S:%]"" DE(11)=%
 I  S %=$P(%Z,U,8) S:%]"" DE(13)=%,DE(16)=% S %=$P(%Z,U,9) S:%]"" DE(21)=% S %=$P(%Z,U,10) S:%]"" DE(19)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X36 N IBDFFLG D SCDEL^IBDF9A3 I $D(IBDFFLG) S Y="@99"
 Q
37 S DW="0;3",DV="NJ3,0",DU="",DLB="SUBCOLUMN WIDTH",DIFLD=.03
 S X=+$G(IBLEN(IBD))
 S Y=X
 G Y
X37 K:+X'=X!(X>150)!(X<1)!(X?.E1"."1N.N) X
 Q
 ;
38 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=38 D X38 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X38 I $P(^IBE(357.2,D0,2,D1,0),U,2)'="" S Y="@11"
 Q
39 S DW="0;2",DV="F",DU="",DLB="SUBCOLUMN HEADER",DIFLD=.02
 S X=$P($G(IBLEN(IBD)),U,2)
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
X39 K:$L(X)>150!($L(X)<1) X
 I $D(X),X'?.ANP K X
 Q
 ;
40 S DQ=41 ;@11
...
X41 S X=X
 Q
42 S DW="0;2",DV="F",DU="",DLB="WHAT TEXT SHOULD APPEAR AT THE TOP OF THE SUBCOLUMN?",DIFLD=.02
 G RE
X42 K:$L(X)>150!($L(X)<1) X
 I $D(X),X'?.ANP K X
 Q
 ;
