PRCATP7 ; GENERATED FROM 'PRCAT NEW AR' PRINT TEMPLATE (#409) ; 09/19/10 ; (FILE 430, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 D N:$X>12 Q:'DN  W ?12 S Y=$P(X,U,8) S Y=$S(Y="":Y,$D(^PRCA(430.3,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,30)
 D N:$X>40 Q:'DN  W ?40 W "CATEGORY:"
 D N:$X>51 Q:'DN  W ?51 S Y=$P(X,U,2) S Y=$S(Y="":Y,$D(^PRCA(430.2,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,30)
...
A1R ;
 ...
 S X=$G(^PRCA(430,D0,9)) D N:$X>21 Q:'DN  W ?21 S Y=$P(X,U,1) S Y=$S(Y="":Y,$D(^VA(200,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,35)
 D N:$X>0 Q:'DN  W ?0 S X="=",DIP(1)=X S X=80,X1=DIP(1) S %=X,X="" Q:X1=""  S $P(X,X1,%\$L(X1)+1)=X1,X=$E(X,1,%) K DIP K:DN Y W X
 K Y
 Q
HEAD ;
