ONCDTX ;Hines OIFO/GWB; Delete treatment data; 08/27/97
 ;;2.11;ONCOLOGY;**13,15,19,22,25,27,36,42,44,45**;Mar 07, 1995
 ;
DEL ;Delete all First Course of Treatment data
 I '$D(DATEDX) Q
 S TXDT=$P(^ONCO(165.5,DA,3.1),U,38)_"S0" ;DATE FIRST SURGICAL PROCEDURE
 ...
 S $P(^ONCO(165.5,DA,3),U,33)=""          ;RECONSTRUCTION/RESTORATION
 ...
 S $P(^ONCO(165.5,DA,3.1),U,39)=""        ;SYSTEMIC/SURGERY SEQUENCE
...
SURR ;SURGERY OF PRIMARY (R) (165.5,58.2)
 ...
 D CHKCOC^ONCATF I (COC=0)!(COC=3)!(COC=6) Q
 S DSATF=1,$P(^ONCO(165.5,D0,3.1),U,7)="" D SPSATFR^ONCDTX1
 Q
 ;
SUR ;SURGERY OF PRIMARY (F) (165.5,58.6)
 ...
 D CHKCOC^ONCATF I (COC=0)!(COC=3)!(COC=6) Q
 S DSATF=1,$P(^ONCO(165.5,D0,3.1),U,30)="" D SPSATF^ONCDTX1
 Q
 ;
SCOPER ;SCOPE OF LN SURGERY (R) (165.5,138)
 ...
 D CHKCOC^ONCATF I (COC=0)!(COC=3)!(COC=6) Q
 S DSCATF=1,$P(^ONCO(165.5,D0,3.1),U,9)="" D SCPATFR^ONCDTX1
 Q
 ;
SCOPE ;SCOPE OF LN SURGERY (F) (165.5,138.4)
 ...
 D CHKCOC^ONCATF I (COC=0)!(COC=3)!(COC=6) Q
 S DSCATF=1,$P(^ONCO(165.5,D0,3.1),U,32)="" D SCPATF^ONCDTX1
 Q
 ;
SOSNR ;SURG PROC/OTHER SITE (R) (165.5,139)
 ...
 D CHKCOC^ONCATF I (COC=0)!(COC=3)!(COC=6) Q
 S DSOATF=1,$P(^ONCO(165.5,D0,3.1),U,10)="" D SOSATFR^ONCDTX1
 Q
 ;
SOSN ;SURG PROC/OTHER SITE (F) (165.5,139.4)
 ...
 D CHKCOC^ONCATF I (COC=0)!(COC=3)!(COC=6) Q
 S DSOATF=1,$P(^ONCO(165.5,D0,3.1),U,34)="" D SOSNATF^ONCDTX1
 Q
 ;
RAD ;RADIATON (165.5,51.2)
 ...
 W !,"  TEXT-RX-RADIATION"
 D CHKCOC^ONCATF I (COC=0)!(COC=3)!(COC=6) Q
 S DRATF=1,$P(^ONCO(165.5,D0,3.1),U,12)="" D RADATF^ONCDTX1
 Q
 ;
CHE ;CHEMOTHERAPY (165.5,53.2)
 ...
 W !,"  RX TEXT-CHEMO"
 D CHKCOC^ONCATF I (COC=0)!(COC=3)!(COC=6) Q
 S DCATF=1,$P(^ONCO(165.5,D0,3.1),U,14)="" D CHEMATF^ONCDTX1
 Q
 ;
HOR ;HORMONE THERAPY (165.5,54.2)
 ...
 W !,"  RX TEXT-HORMONE"
 D CHKCOC^ONCATF I (COC=0)!(COC=3)!(COC=6) Q
 S DHATF=1,$P(^ONCO(165.5,D0,3.1),U,16)="" D HORATF^ONCDTX1
 Q
 ;
IMM ;IMMUNOTHERAPY (165.5,55.2)
 ...
 W !,"  RX TEXT-BRM"
 D CHKCOC^ONCATF I (COC=0)!(COC=3)!(COC=6) Q
 S DIATF=1,$P(^ONCO(165.5,D0,3.1),U,18)="" D IMMATF^ONCDTX1
 Q
 ;
...
OTH ;OTHER TREATMENT (165.5,57.2)
 ...
 W !,"  RX TEXT-OTHER"
 D CHKCOC^ONCATF I (COC=0)!(COC=3)!(COC=6) Q
 S DOATF=1,$P(^ONCO(165.5,D0,3.1),U,20)="" D OTHATF^ONCDTX1
 Q
 ;
...
NCDS ;SURGICAL DX/STAGING PROC (165.5,58.1)
 ...
 D CHKCOC^ONCATF I (COC=2)!(COC=3) Q
 S DNCATF=1,$P(^ONCO(165.5,D0,3.1),U,5)="" D NCDSATF^ONCDTX1
 Q
 ;
SCT ;SUBSEQUENT COURSE OF TREATMENT (surgery sub-fields)
