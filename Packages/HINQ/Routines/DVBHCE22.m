...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 ...
 I $D(^(.31)) S %Z=^(.31) S %=$P(%Z,U,3) S:%]"" DE(1)=% S %=$P(%Z,U,4) S:%]"" DE(45)=%
 I $D(^(.35)) S %Z=^(.35) S %=$P(%Z,U,1) S:%]"" DE(20)=%
 I $D(^(.52)) S %Z=^(.52) S %=$P(%Z,U,5) S:%]"" DE(38)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X37 D POW^DVBHUTIL
 Q
38 D:$D(DG)>9 F^DIE17,DE S DQ=38,DW=".52;5",DV="RSX",DU="",DLB="POW STATUS INDICATED?",DIFLD=.525
 S DE(DW)="C38^DVBHCE22",DE(DW,"INDEX")=1
...
C38S S X="" G:DG(DQ)=X C38F1 K DB
 D ^DVBHCE23
C38F1 N X,X1,X2 S DIXR=646 D C38X1(U) K X2 M X2=X D C38X1("O") K X1 M X1=X
 D
