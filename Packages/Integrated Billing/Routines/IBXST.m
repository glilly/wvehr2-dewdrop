IBXST ; GENERATED FROM 'IB STATUS' INPUT TEMPLATE(#506), FILE 399;09/19/10
 D DE G BEGIN
DE S DIE="^DGCR(399,",DIC=DIE,DP=399,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DGCR(399,DA,""))=""
 ...
 I $D(^("TX")) S %Z=^("TX") S %=$P(%Z,U,2) S:%]"" DE(13)=%,DE(19)=% S %=$P(%Z,U,5) S:%]"" DE(22)=% S %=$P(%Z,U,6) S:%]"" DE(10)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
7 D:$D(DG)>9 F^DIE17,DE S DQ=7,D=0 K DE(1) ;44
 S DIFLD=44,DGO="^IBXST1",DC="1^399.044PA^D1^",DV="399.044MP399.4'",DW="0;1",DOW="REASON(S) DISAPPROVED-INITIAL",DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
 S DU="DGCR(399.4,"
...
X21 I $P($G(^DGCR(399,DA,"TX")),U,5)="C" S Y="@99"
 Q
22 D:$D(DG)>9 F^DIE17,DE S DQ=22,DW="TX;5",DV="S",DU="",DLB="CLAIM MRA STATUS",DIFLD=24
 S DE(DW)="C22^IBXST"
