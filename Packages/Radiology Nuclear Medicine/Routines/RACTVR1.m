...
DE S DIE="^RARPT(",DIC=DIE,DP=74,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^RARPT(DA,""))=""
 I $D(^(0)) S %Z=^(0) S %=$P(%Z,U,5) S:%]"" DE(4)=% S %=$P(%Z,U,7) S:%]"" DE(14)=% S %=$P(%Z,U,9) S:%]"" DE(7)=% S %=$P(%Z,U,10) S:%]"" DE(9)=% S %=$P(%Z,U,17) S:%]"" DE(1)=%,DE(10)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
1 S DW="0;17",DV="P200'",DU="",DLB="STATUS CHANGED TO VERIFIED BY",DIFLD=17
 S DU="VA(200,"
 S X=DUZ
 S Y=X
...
X2 S Y="@30"
 Q
3 S DQ=4 ;@15
4 S DW="0;5",DV="RSX",DU="",DLB="REPORT STATUS",DIFLD=5
 S DE(DW)="C4^RACTVR1"
...
X5 S Y="@5"
 Q
6 S DQ=7 ;@20
7 D:$D(DG)>9 F^DIE17,DE S DQ=7,DW="0;9",DV="*P200'X",DU="",DLB="VERIFYING PHYSICIAN",DIFLD=9
 S DU="VA(200,"
 S X=RAVER
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
X7 S DIC("S")="N RAINADT X ^DD(74,9,9.2) I $S('RAINADT:1,DT'>RAINADT:1,1:0),$D(^XUSEC(""RA VERIFY"",+Y)),($D(^VA(200,""ARC"",""R"",+Y))!($D(^VA(200,""ARC"",""S"",+Y))))" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
8 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=8 D X8 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X8 S X1=RASIG("PER"),X=RASIG("NAME"),X2=DA D EN^XUSHSHP S RASIGCDE=X
 Q
9 S DW="0;10",DV="FIO",DU="",DLB="ELECTRONIC SIGNATURE CODE",DIFLD=10
 S DQ(9,2)="S Y(0)=Y S Y=""  <Hidden>"""
 S X=RASIGCDE
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
 G RD
X9 K:$L(X)>50!($L(X)<1) X
 ...
 Q
 ;
10 S DW="0;17",DV="P200'",DU="",DLB="STATUS CHANGED TO VERIFIED BY",DIFLD=17
 S DU="VA(200,"
 S X=DUZ
 S Y=X
 S X=Y,DB(DQ)=1,DE(DW,"4/")="" G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
X11 K RASIGCDE,X2,X1
 Q
12 S DQ=13 ;@30
13 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=13 G A
14 S DW="0;7",DV="DI",DU="",DLB="VERIFIED DATE",DIFLD=7
 S DE(DW)="C14^RACTVR1"
 S X="NOW"
 S Y=X
 S X=Y,DB(DQ)=1 G:X="" N^DIE17:DV,A I $D(DE(DQ)),DV["I"!(DV["#") D E^DIE0 G A:'$D(X)
...
X16 I $E(RAOLDST)=$E(RACT) S Y="@999"
 Q
17 D:$D(DG)>9 F^DIE17,DE S DQ=17,D=0 K DE(1) ;100
 S DIFLD=100,DGO="^RACTVR2",DC="10^74.01DA^L^",DV="74.01RD",DW="0;1",DOW="LOG DATE",DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
 I $D(DSC(74.01))#2,$P(DSC(74.01),"I $D(^UTILITY(",1)="" X DSC(74.01) S D=$O(^(0)) S:D="" D=-1 G M17
 S D=$S($D(^RARPT(DA,"L",0)):$P(^(0),U,3,4),$O(^(0))'="":$O(^(0)),1:-1)
M17 I D>0 S DC=DC_D I $D(^RARPT(DA,"L",+D,0)) S DE(17)=$P(^(0),U,1)
