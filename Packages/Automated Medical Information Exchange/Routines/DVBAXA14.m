...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 ...
 I $D(^(.13)) S %Z=^(.13) S %=$P(%Z,U,1) S:%]"" DE(5)=% S %=$P(%Z,U,2) S:%]"" DE(6)=%
 I $D(^(.32)) S %Z=^(.32) S %=$P(%Z,U,6) S:%]"" DE(8)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW=".11;4",DV="FaR",DU="",DLB="CITY",DIFLD=.114
...
X1 K:$L(X)>15!($L(X)<2) X
 ...
 Q
 ;
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW=".11;5",DV="*P5'aR",DU="",DLB="STATE",DIFLD=.115
...
X2 S DIC("S")="I $P(^DIC(5,Y,0),U,6)=1" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
3 D:$D(DG)>9 F^DIE17,DE S DQ=3,DW=".11;12",DV="FXOaR",DU="",DLB="ZIP+4",DIFLD=.1112
 S DQ(3,2)="S Y(0)=Y D ZIPOUT^VAFADDR"
 S DE(DW)="C3^DVBAXA14",DE(DW,"INDEX")=1
 G RE
...
X3 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>20!($L(X)<5) X I $D(X) D ZIPIN^VAFADDR
 ...
 Q
 ;
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW=".11;7",DV="NJ3,0XOaR",DU="",DLB="COUNTY",DIFLD=.117
...
X5 K:$L(X)>20!($L(X)<4) X
 ...
 Q
 ;
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW=".13;2",DV="Fa",DU="",DLB="PHONE NUMBER [WORK]",DIFLD=.132
 S DE(DW)="C6^DVBAXA14"
 G RE
...
X7 S DVBACPLT=1
 Q
8 D:$D(DG)>9 F^DIE17,DE S DQ=8,DW=".32;6",DV="RDX",DU="",DLB="SERVICE ENTRY DATE [LAST]",DIFLD=.326
 S DE(DW)="C8^DVBAXA14",DE(DW,"INDEX")=1
 G RE
...
C8F1 S DIEZRXR(2,DIIENS)=$$OREF^DILF($NA(@$$CREF^DILF(DIE)))
 ...
 Q
X8 S %DT="E",%DT(0)=-DT D ^%DT K %DT S X=Y K:Y<1 X I $D(X) S DFN=DA D SV^DGLOCK I $D(X) K:'$$VALMSE^DGRPMS(DFN,X,0,"MSL") X I $D(X) S DGFRDT=X I $D(^DG(43,1)) S SD1=1 D POS^DGINP
 Q
 ;
9 D:$D(DG)>9 F^DIE17 G ^DVBAXA22
