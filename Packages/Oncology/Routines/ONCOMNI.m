ONCOMNI ;HIRMFO/GWB,RTK-ALGORITHM FOR COMPUTING MIDDLE NAME/INITIAL ;12/10/99
 ;;2.11;ONCOLOGY;**1,11,13,22,25,28**;Mar 07, 1995
 ;
 D SETUP^ONCOES
...
CHFPH ;CALCULATE VALUE OF FIELD #804 (CANCER HISTORY-1ST PRIMARY HISTOLOGY)
 ...
 .I CHFH=D0 Q
 .S CHFHFLG=CHFHFLG+1,TPX=$$HIST^ONCFUNC(CHFH) S:TPX'="" TPX=$G(^ONCO(ICDFILE,TPX,0)) S:TPX'="" X=$P(TPX,U,2) Q
 K CHFH,CHFHFLG,CHFHNM,TPX Q
...
CHSPH ;CALCULATE VALUE OF FIELD #806 (CANCER HISTORY-2ND PRIMARY HISTOLOGY)
 ...
 .S CHSHFLG=CHSHFLG+1,TPX=$$HIST^ONCFUNC(CHSH) S:TPX'="" TPX=$G(^ONCO(ICDFILE,TPX,0)) S:TPX'="" X=$P(TPX,U,2) Q
...
RCSHP ;RADIATION/CHEMOTHERAPY SEQUENCE (#862) HELP
 ...
 Q
 S %DT="EP",%DT(0)="-NOW" D ^%DT S X=Y K:Y<1 X K ^%DT(0)
...
SSC ;Calculate default for fields #1104,#1105
 ;I $P($G(^ONCO(165.5,D0,"MEL1")),U,5)'="" S X="" Q
