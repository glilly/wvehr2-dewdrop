...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 I $D(^(.11)) S %Z=^(.11) S %=$P(%Z,U,1) S:%]"" DE(6)=% S %=$P(%Z,U,2) S:%]"" DE(7)=% S %=$P(%Z,U,3) S:%]"" DE(8)=% S %=$P(%Z,U,4) S:%]"" DE(9)=%
...
C1S S X="" Q:DG(DQ)=X  K DB
 ...
 Q
X1 Q
2 D:$D(DG)>9 F^DIE17,DE S DQ=2,DW="VET;1",DV="RSXa",DU="",DLB="VETERAN (Y/N)?",DIFLD=1901
 S DE(DW)="C2^DVBAXA7"
 S DU="Y:YES;N:NO;"
 G RE
...
X8 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>30!($L(X)<3) X
 ...
 Q
 ;
9 D:$D(DG)>9 F^DIE17,DE S DQ=9,DW=".11;4",DV="FaR",DU="",DLB="CITY",DIFLD=.114
 S DE(DW)="C9^DVBAXA7"
 G RE
...
C9S S X="" Q:DG(DQ)=X  K DB
 D ^DVBAXA15
 Q
X9 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>15!($L(X)<2) X
 I $D(X),X'?.ANP K X
 Q
 ;
10 D:$D(DG)>9 F^DIE17 G ^DVBAXA16
