ICD1845A ;ALB/MJB - FY 2010 DRG UPDATE ;
 ;;18.0;DRG Grouper;**45**;Oct 13, 2000;Build 14
 ;
 ;Update the (#80.2) DRG file with FY 2010 DRG Grouper MS-DRG codes.
 ;The last DRG code filed for FY 2009 was DRG999.
 ;
 Q
 ;
 ;Routines ICD1845* contain each FY 2010 MS-DRG code update values
...
DRG ;post-install driver (#80.2) DRG updates
 ...
 ;element values used to create the FY 2010 MS-DRG updates.
 ; Input:
 ...
 ;   ICDTOT - Total MS-DRG codes filed
 ;
 D BMES^XPDUTL(">>> Adding FY 2010 DRG Grouper updates to (#80.2) DRG file...")
 N ICDRTN,ICDI,ICDSUB,ICDEDIT,ICDADD,ICDTMP
 S (ICDEDIT,ICDADD)=0
 S ICDTOT=$G(ICDTOT) I ICDTOT']"" S ICDTOT=0
 S ICDTMP=$G(ICDTMP)
 I ICDTMP']"" S ICDTMP=$NA(^TMP("DRGFY2010",$J)) D
 . K @ICDTMP
 . S @ICDTMP@(0)="PATCH FY 2010 DRG UPDATE^"_$$NOW^XLFDT
 ;
 ;loop each sub-routine
 S ICDSUB="FGHIJK"
 F ICDI=1:1:6 S ICDRTN="^ICD1845"_$E(ICDSUB,ICDI) D
  .Q:($T(@ICDRTN)="")
...
EDITDRG(ICDLINE,ICDTMP,ICDTOT,ICDEDIT) ; edit existing (#80.2) DRG record
 ...
 S ICDREF="ICDTBL"_$S(ICDDRG<100:"0",1:$E(ICDDRG,1))_"B" ;ECF new line
