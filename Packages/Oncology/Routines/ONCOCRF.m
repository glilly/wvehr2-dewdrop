ONCOCRF ;Hines OIFO/GWB - FOLLOW-UP ;07/13/00
 ;;2.11;ONCOLOGY;**6,11,16,22,25,26,28,44,45**;Mar 07, 1995
LD ;Get node for DATE OF LAST CONTACT OR DEATH (160.04,.01)
...
SLF ;Set DATE OF LAST CONTACT OR DEATH (160.04,.01) "AA" cross-reference
 ...
 I $D(ONCDUZ) S $P(^ONCO(160,XD0,"F",DA,0),U,10)=ONCDUZ
 I $D(ONCDT) S $P(^ONCO(160,XD0,"F",DA,0),U,11)=ONCDT
 G EX
 ;
...
UPD ;Update the following fields with the most recent FOLLOW-UP (160,400)
 ...
 S $P(^ONCO(160,XD0,1),U,1)=ONCOVS,^ONCO(160,"AS",ONCOVS,XD0)=""
 S $P(^ONCO(160,XD0,1),U,7)=FS,$P(^(1),U,4)=$S(ONCOVS=0:9,1:0)
