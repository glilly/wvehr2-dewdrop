PRCATP6 ; GENERATED FROM 'PRCAP RETURN BILL' PRINT TEMPLATE (#416) ; 09/19/10 ; (FILE 430, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 S X=$G(^PRCA(430,D0,9)) D N:$X>14 Q:'DN  W ?14 S Y=$P(X,U,6) S Y=$S(Y="":Y,$D(^PRCA(430.3,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,30)
 D N:$X>39 Q:'DN  W ?39 W "CURR. STATUS:"
 S X=$G(^PRCA(430,D0,0)) D N:$X>54 Q:'DN  W ?54 S Y=$P(X,U,8) S Y=$S(Y="":Y,$D(^PRCA(430.3,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,30)
 ...
 S X=$G(^PRCA(430,D0,100)) D N:$X>48 Q:'DN  W ?48 S Y=$P(X,U,2) S Y=$S(Y="":Y,$D(^DIC(49,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,30)
 D T Q:'DN  D N D N:$X>33 Q:'DN  W ?33 W "<< SERVICE >>"
 D N:$X>0 Q:'DN  W ?0 W "APPROV. BY:"
 S X=$G(^PRCA(430,D0,104)) D N:$X>12 Q:'DN  W ?12 S Y=$P(X,U,1) S Y=$S(Y="":Y,$D(^VA(200,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,35)
 ...
 S X=$G(^PRCA(430,D0,3)) D N:$X>12 Q:'DN  W ?12 S Y=$P(X,U,2) S Y=$S(Y="":Y,$D(^VA(200,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,35)
 D N:$X>39 Q:'DN  W ?39 W "DATE:"
