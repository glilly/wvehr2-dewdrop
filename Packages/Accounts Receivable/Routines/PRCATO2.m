PRCATO2 ; GENERATED FROM 'PRCA DISP AUDIT' PRINT TEMPLATE (#395) ; 09/19/10 ; (FILE 430, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 W ?41 S Y=$P(X,U,2) S Y=$S(Y="":Y,$D(^PRCA(430.2,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,30)
 D N:$X>41 Q:'DN  W ?41 S Y=$P(X,U,16) S Y=$S(Y="":Y,$D(^PRCA(430.2,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,30)
 ...
 S X=$G(^PRCA(430,D0,104)) W ?11 S Y=$P(X,U,1) S Y=$S(Y="":Y,$D(^VA(200,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,35)
