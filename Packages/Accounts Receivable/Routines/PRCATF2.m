PRCATF2 ; GENERATED FROM 'PRCA FMS TRANS STAT' PRINT TEMPLATE (#443) ; 09/19/10 ; (FILE 347, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 S X=$G(^RC(347,D0,0)) S Y=$P(X,U,8) S Y=$S(Y="":Y,$D(^PRCA(433,Y,0))#2:$P(^(0),U),1:Y) W:Y]"" $J(Y,7,0)
...
A1R ;
 ...
 G B1R
B1 ;
 S X=$G(^PRCA(433,D0,1)) S Y=$P(X,U,2) S Y=$S(Y="":Y,$D(^PRCA(430.3,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,30)
 Q
B1R ;
 K J(100),I(100) S:$D(I(0,0)) D0=I(0,0)
 F Y=0:0 Q:$Y>4  W !
