IMRSUDOP ;ISC-SF/JLI,HCIOFO/FT/FAI/SPS-LOCAL COUNT OF PTS, STATUS, OP VISITS, IP STAYS, ETC. ;07/23/01  07:28
...
C1 S IMRSDV=0,IMRSDVA=1,DFN=IMRDFN D DEM^VADPT
 ...
 S VASD("F")=IMRSD,VASD("T")=IMRED D SDA^VADPT K DFN,VASD
 F IMRDY=0:0 S IMRDY=$O(^UTILITY("VASD",$J,IMRDY)) Q:IMRDY'>0  S IMRD=+^(IMRDY,"I"),IMRCS=$P(^("I"),U,2) D C2
 K ^UTILITY("VASD",$J)
 S IMRSDV=1
 F IMRD=IMRSD-.0000001:0 S IMRD=$O(^SDV("C",IMRDFN,IMRD)) Q:IMRD'>0!(IMRD'<(IMRED+1))  S IMRSDVA=$S('$D(^TMP($J,IMR1C,"PAT",IMRDFN,"S",(IMRD\1))):1,1:0) S IMRSDVI=IMRD D SDVCS^IMRUTL I $D(IMRAR(409.51)) D
