MCAROT ; GENERATED FROM 'MCARETT1' PRINT TEMPLATE (#975) ; 09/19/10 ; (FILE 691.7, MARGIN=80)
 G BEGIN
N W !
...
C1R ;
 ...
 G D1R
D1 ;
 S X=$G(^MCAR(691.7,D0,1,D1,0)) W ?17 S Y=$P(X,U,1) S Y=$S(Y="":Y,$D(^MCAR(695,Y,0))#2:$P(^(0),U),1:Y) S Y=$S(Y="":Y,$D(^PSDRUG(Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,30)
 W " "
 W ?0,$E($P(X,U,2),1,10)
 W " "
 W ?0,$E($P(X,U,3),1,10)
