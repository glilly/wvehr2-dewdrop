ONCOX11 ; GENERATED FROM 'ONCOX11' PRINT TEMPLATE (#852) ; 09/19/10 ; (FILE 165.5, MARGIN=80)
 G BEGIN
N W !
...
BEGIN ;
 ...
 S X=$G(^ONCO(165.5,D0,7)) S Y=$P(X,U,6) S Y=$S(Y="":Y,$D(^ONCO(164.42,Y,0))#2:$P(^(0),U),1:Y) W $E(Y,1,40)
...
A2R ;
 ...
 G B2R
B2 ;
 S X=$G(^ONCO(160,D0,4,D1,0)) S DIWL=18,DIWR=78 D ^DIWP
 Q
B2R ;
 D 0^DIWW
 D ^DIWW
